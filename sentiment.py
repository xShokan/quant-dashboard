...
def fetch_news(code: str, name: str, limit: int = 15) -> list:
    """拉取个股最新新闻, 与本地缓存合并(按链接去重), 返回 [{time,title,link,score,label,reason}]"""
        ...
        for _, row in df.head(limit).iterrows():
            ...
            if link not in by_link:
                by_link[link] = {
                    ...
                    "link": link, "score": None, "label": None, "reason": None,
                }
    except Exception as e:
        print(f"  {code} {name} 新闻拉取失败: {type(e).__name__}")
    news = sorted(by_link.values(), key=lambda n: n["time"], reverse=True)[:limit]
...
def deepseek_score(code: str, name: str, news: list) -> int:
    ...
    if not key:
        return 0
    titles = "\n".join(f"{i}. {n['title']}" for i, n in todo)
    resp = requests.post(
        API_URL,
